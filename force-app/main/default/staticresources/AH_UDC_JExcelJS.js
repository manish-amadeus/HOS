...
(function(global, factory) {
   typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory() : typeof define === 'function' && define.amd ? define(factory) : global.jspreadsheet = global.jexcel = factory();
}(this, (function() {
   'use strict';
   ...
   var jexcel = (function(el, options) {
      ...
      var defaults = {
         ...
         text: {
            ...
            copy: 'Copy...',
            paste: navigator.userAgent.toLowerCase().indexOf('firefox') <= -1 ? 'Paste...' : 'Use Ctrl + V to paste', //Paste doesn't work in FF using right click menu
            saveAs: 'Save as...',
            about: 'About',
      ...
      obj.createTable = function() {
         ...
         // Create table container TO DO: frozen columns
      ...
      obj.setRowData = function(rowNumber, data) {
         for (var i = 0; i < obj.headers.length; i++) {
            ...
            // Set value
            if (data[i] != null) {
               obj.setValue(columnName, data[i], true); //789836 : Passing data from Amadeus Form to Amadeus Form - build API - Added true to force update readonly columns 
            }
         }
      }
      /**
      ...
      obj.createCell = function(i, j, value) {
         ...
         // Readonly
         if (obj.options.columns[i].readOnly == true || obj.options.columns[i].readOnly == 'true') { //789836 : Passing data from Amadeus Form to Amadeus Form - build API
      ...
      obj.openFilter = function(columnId) {
         if (!obj.options.filters) {
            ...
            //Fixed BUG 751873: All filter options not being showed in Firefox browser
            ...
            for (var i = 0; i < query.length; i++) {
               if ((query[i] == '' && // Blank matching
                  ...
                  ((query[i] != '' && // Normal non-blank filtering
                     (('' + obj.options.data[y][x]).search(query[i]) >= 0 || ('' + obj.records[y][x].innerHTML).search(query[i]) >= 0)))) {
                  return true;
               }
            }
            return false;
         ...
         if (obj.records[y][x].classList.contains('readonly') == true && !force) {
            ...
            if (obj.options.columns[x].editor) {
               ...
               if (obj.options.columns[x].type == 'checkbox' || obj.options.columns[x].type == 'radio') {
                  ...
                  // Update data and cell                                  
                  ////Start : USER STORY 743356 - Point 6 : Pasting values into the Grid
                  // obj.options.data[y][x] = value;
                  // obj.records[y][x].innerText = obj.getDropDownValue(x, value);

                  var fi = obj.options.columns[x].source.findIndex(({ name }) => name == value);
                  if(fi < 0)
                  {
                     fi = obj.options.columns[x].source.findIndex(({ id }) => id == value);
                     if(fi < 0)
                     {
                  ...
                  if(fi >= 0)
                  {
                     obj.options.data[y][x] = obj.options.columns[x].source[fi].id;
                     obj.records[y][x].innerText = obj.options.columns[x].source[fi].name;
                  }                
                  ////End : USER STORY 743356 - Point 6 : Pasting values into the Grid                 
         ...
         for (var j = y1; j <= y2; j++) {
            ...
            for (var i = x1; i <= x2; i++) {
               ...
               if (obj.records[j][i] && !obj.records[j][i].classList.contains('readonly') && obj.records[j][i].style.display != 'none' && breakControl == false) {
                  ...
                  //Start : USER STORY 718778 - Point 6 : Fixed drag row issue when hidden column is there
                  /*if (data[posy] == undefined) {
                     posx = 0;
                  ...
                  if (data[posy] == undefined || data[posy][posx] == undefined) {
                     continue;
                  }
                  //End : USER STORY 718778 - Point 6 : Fixed drag row issue when hidden column is there
                  ...
                  posx++; //USER STORY 718778 - Point 6 : Fixed - drag row issue when hidden column is there

                  //USER STORY 722401 - Point 1 : Fixed - In case of row drag functionality, Long text field is displaying data with new lines hence it is increasing cell height
                  if(typeof(obj.options.columns[i].contenttype) != 'undefined' && obj.options.columns[i].contenttype != null
                     && obj.options.columns[i].contenttype.toLowerCase() == 'longtext') {
                     var cellName = jexcel.getColumnNameFromId([i, j]);
                     //Remove new lines and set text on long text field
                     setLongTextCellText(obj.getCell(cellName), value);
                  }
               }
               //posx++; //USER STORY 718778 - Point 6 : Fixed drag row issue when hidden column is there
               if (h[0] != x1) {
      ...
      obj.updateScroll = function(direction) {
         ...
         // Left position check - TO DO: change that to the bottom border of the element
      ...
      obj.copy = function(highlighted, delimiter, returnData, includeHeaders, download) {
         ...
         var numOfCols = 0;
         var numOfRows = 0;
         if ((download && obj.options.includeHeadersOnDownload == true) || (!download && obj.options.includeHeadersOnCopy == true) || (includeHeaders)) {
            // Nested headers
            if (obj.options.nestedHeaders && obj.options.nestedHeaders.length > 0) {
               // Flexible way to handle nestedheaders
               if (!(obj.options.nestedHeaders[0] && obj.options.nestedHeaders[0][0])) {
      ...
      obj.init = function() {
         ...
         el.addEventListener('focus', function(e) {
            //USER STORY 718778 - Fixed issue - Filters are not working for right side columns after scrolling horizontally
            if(e.relatedTarget != null && $(e.relatedTarget).hasClass('jdropdown-header')
               && $(e.relatedTarget).parents('.jexcel_column_filter').length > 0) { //Only if clicked on filter dropdown
               //Ignore focus when filter drop down item is clicked
               return;
            }
         ...
         obj.options.contextMenu = function(el, x, y, e) {
            ...
            if (navigator && navigator.clipboard) {
               items.push({
                  ...
                  shortcut: 'Ctrl + V',
                  disabled: navigator.userAgent.toLowerCase().indexOf('firefox') > -1, //Disable paste option for FF browser
                  onclick: function() {
                     if (obj.selectedCell) {
      ...
      obj.scrollControls = function(e) {
         ...
         // Close editor
         //UDC BUG 751873 - AmadeusForm Grid - Filter issue in Firefox browser
         if (obj.options.lazyLoading == true || obj.options.tableOverflow == true) {
            if (e.target.className.substr(0, 9) != 'jdropdown') {
               if (obj.edition) {
                  if ($('.jdropdown:visible').parents('.jexcel_dropdown').length > 0) { //If dropdown is opened in cell
                     //Calculate time duration of dropdown opening
                     var intSeconds = Math.floor(window.performance.now() - jSuites.dropdownOpenTimeStamp) / 1000;
                     if (intSeconds > 1) { //Fixed issue: dropdown not opens in last column
                        obj.closeEditor(obj.edition[0], true);
                     }
                  } else {
                     obj.closeEditor(obj.edition[0], true);
                  }
               } else if ($('.jdropdown:visible').parents('.jexcel_column_filter').length > 0) {
                  var intSeconds = Math.floor(window.performance.now() - jSuites.dropdownOpenTimeStamp) / 1000;
                  if (intSeconds > 1) { //Fixed issue: filter dropdown not open in last column
                     //Close filter filter dropdown if it is opened
                     $('.jdropdown:visible').get(0).dropdown.close();
                  }
               }
            }
         }
      ...
      if (jexcel.current) {
         if (jexcel.current.edition) {
            ...
            if (jexcel.current) {
               ...
               var y = e.target.getAttribute('data-y');
               if (x || y) {
                  if ((x < parseInt(jexcel.current.selectedCell[0])) || (x > parseInt(jexcel.current.selectedCell[2])) || (y < parseInt(jexcel.current.selectedCell[1])) || (y > parseInt(jexcel.current.selectedCell[3]))) {
