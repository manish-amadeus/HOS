...
(function(global, factory) {
   ...
   'use strict';
   var sidebarId = 'opened-sidebar-div';
   var sidebarClassId = 'opened-sidebar.content';
   var jSuites = function(options) {
      ...
      obj.current = [];
      obj.dropdownOpenTimeStamp = null;
      return obj;
   }();
   /**
               ...
               if (jSuites.getWindowWidth() < 800 || obj.options.fullscreen) {
                  ...
                  if (obj.options.position) {
                     ...
                     if (window.innerHeight < rect.bottom + rectContent.height) {
                        intTop = (rect.top - (rectContent.height + 2 ));
                        calendarContainer.style.top = intTop + 'px';
                     } else {
                        intTop = rect.top + rect.height + 2;
                        calendarContainer.style.top = intTop + 'px';
                     }
                    
                     //*UDC:Bug 755688:Calendar is not properly positioned on screen for date column in Grid - Start
                     var sbOffsetLeft = $('#'+sidebarId).offset().left;                     
                     if ($('.'+sidebarClassId).length > 0 && rect.left > sbOffsetLeft) { //If side bar is open in the UDC community page then set correct position
                        calendarContainer.style.left = (rect.left - sbOffsetLeft) + 'px';
                        var sbOffsetTop = $('#'+sidebarId).offset().top;
                        calendarContainer.style.top = (intTop + window.pageYOffset - sbOffsetTop) + 'px';
                        //End
                     } else {
   ...
   jSuites.contextmenu = (function(el, options) {
      ...
      obj.open = function(e, items) {
         ...
         // Coordinates
         /**** This commented code is updated with below code
         ...
         //START - USER STORY 722401 - Point 2 : Fixed right click dialog issue for different screen resolution
         if ((obj.options.items && obj.options.items.length > 0) || el.children.length) {
            if (e.target) {
               var x = e.pageX - $('#'+sidebarId).offset().left;
               var y = e.clientY - $('#'+sidebarId).offset().top - document.getElementById('logo').offsetHeight;
            }
            else
            {
               var x = e.x;
            ...
            el.style.top = y + 'px';
            el.style.left = x + 'px';
            if ($('.'+sidebarClassId).length > 0) {
               var intTop = e.pageY - $('#'+sidebarId).offset().top - document.getElementById('logo').offsetHeight;
               el.style.top = intTop + 'px';
               var intLeft = (x -  rect.width) + 'px';
               var sbOffset =  $('.sidebar .left-side').width() + $('.sidebar .right-side').width()  + 10 ;
               el.style.left = (intLeft - (intLeft > sbOffset ? sbOffset : 0)) + 'px';   //$('.sidebar .left-side').width() : 280 ,$('.sidebar .right-side').width() : 50 + 10 padding
            }
            
            if ((window.innerWidth - x - rect.width - $('.sidebar .right-side').width()) < $('#'+sidebarId).offset().left) 
            {
               el.style.left = (x - rect.width + $('.sidebar .right-side').width()) + 'px';
            }
         }
         //END -  USER STORY 722401 - Point 2 : Fixed right click dialog issue for different screen resolution
      }
      /**
       * Close menu
       */
      obj.close = function() {
         ...
         //USER STORY 722401 : Point 2 - Close right click menu on scroll
         if($('.jcontextmenu-focus').length > 0) {
            $('.jcontextmenu-focus').removeClass('jcontextmenu-focus');
         }
      }
      /**
   ...
   jSuites.dropdown = (function(el, options) {
      ...
      obj.open = function() {
         // Focus
         if (!el.classList.contains('jdropdown-focus')) {
            jSuites.dropdownOpenTimeStamp = window.performance.now();
            // Current dropdown
            ...
            //BUG 757006 - Long picklist values extend past column
            if($(el).parents('.jexcel_column_filter').length > 0) { //Only for filter dropdown
               var containerWidth = $('body').width() - $('#left-bluebar-closed').width(); //Get total width of container div
               var elemWidth = $(el).offset().left + $('.jdropdown-container', el).width(); //Get element width
               if(elemWidth > containerWidth) { //If text goes outside then display ...
                  var intWidth = containerWidth - $(el).offset().left; //Total container width - left position of dropdown
                  $('.jdropdown-container', el).css('max-width', intWidth + 'px');
               }
            }
            ...
            if (!obj.options.type || obj.options.type == 'default') {
               ...
               //fixed for the correct position of dropdown and filter
               if (obj.options.position) {
                  container.style.position = 'fixed';

                  if ($('.'+sidebarClassId).length > 0) {
                     var sbLeft = $('#'+sidebarId).offset().left;
                     var sbTop = $('#'+sidebarId).offset().top - $(el).height(); 
                     if ($(container).parents('.jexcel_dropdown').length > 0) {
                        var intTop = $(container).parents('.jexcel_dropdown').offset().top;
                        var intLeft = $(container).parents('.jexcel_dropdown').offset().left;
                        container.style.top = (intTop - (intTop > sbTop ? sbTop : 0)) + 'px';
                        container.style.left = (intLeft - (intLeft > sbLeft ? sbLeft : 0)) + 'px';
                     } else {
                        container.style.top = (rect.top - sbTop) + 'px';
                        container.style.left = (rect.left - (rect.left > sbLeft ? sbLeft : 0)) + 'px';
                     }
                     if ($(container).parents('.jexcel_column_filter').length > 0) {
