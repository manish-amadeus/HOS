...
trigger psa411ReportAi_RunExtract on PSA_411_Report__c (after insert) 
{
    ...
    // A batch run will have set the "Run as Scheduled Batch" value to true, so the indicator that the report is being created from the UI is that it is false.  If false, run the batch report extract.
