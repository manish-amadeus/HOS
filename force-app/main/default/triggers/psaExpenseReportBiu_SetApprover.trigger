...
Description     : When an expense report is saved (but before it is approved) this sets the
				...
				: 
				: TEST CLASS: PSA_411_Report_Test
******************************************************************************************/
trigger psaExpenseReportBiu_SetApprover on pse__Expense_Report__c (before insert, before update) 
{   
    
	// Expenses submitted against Projects whose name starts with "Internal Expenses" will be treated as internal expenses and routed to the submitter's manager. 
    ...
    Map<Id, pse__Proj__c> projects = new Map<Id, pse__Proj__c>([SELECT Name, pse__Region__c, pse__Region__r.Central_Expense_Approver__c FROM pse__Proj__c WHERE Id IN : projectIds]);
        
    // Look up associated resources to get Resource Region which will provide central expense approver
    Map<Id, Contact> resources = new Map<Id, Contact>([SELECT pse__Region__r.Name, pse__salesforce_user__r.ManagerId FROM Contact WHERE Id IN : resourceIds]);
    ...
    for (pse__Expense_Report__c er: Trigger.new) 
    {
    	...
    	catch (Exception e) { er.addError('Error setting expense report approver: ' + e.getMessage()); }
